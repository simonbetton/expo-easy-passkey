...
The example app includes placeholder signing and relying-party values. Before running it on iOS, replace them with values you control:
...
5. In `apps/example/App.tsx`, set `origin` to `https://your-domain` and set `rp.id` and `rpId` to the same domain without `https://`.
...
The committed `appleTeamId`, bundle ID, and `example.com` values are only placeholders for local development. They will not work for other Apple developer accounts.
