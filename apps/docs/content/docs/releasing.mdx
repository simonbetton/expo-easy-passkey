...
- Confirm CI is green, including `pnpm check` and `pnpm verify:release-acceptance`.
...
`pnpm verify` already includes `pnpm check`, which runs the acceptance Jest files. `pnpm test:acceptance` is the local subset if you want to re-run only those suites.
