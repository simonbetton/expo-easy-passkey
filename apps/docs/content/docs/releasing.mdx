...
Publication cannot fall back to checkout binaries: prepare replaces them before `pack:check` and `changeset publish`. npm provenance remains enabled via `publishConfig.provenance` and the workflow `id-token: write` permission, so the published package is attested to this workflow run and commit.
