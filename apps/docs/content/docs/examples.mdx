...
On Android emulators, sign in with a Google account and set a screen lock before testing passkeys. Credential Manager will not offer create/assert options without a passkey-capable account.
...
For local development, the backend can run on your machine, but real native passkey ceremonies still need `rp.id` to remain the associated domain. iOS uses the HTTPS relying-party origin; Android Credential Manager derives an APK-key-hash origin from the installed signing certificate. Keep `PASSKEY_RP_ID` set to your associated domain, configure every trusted Android fingerprint on the backend, and use a tunnel or hosted URL if the device must reach your local API.
...
export function AddPasskeyButton() {
  ...
  const addPasskey = async () => {
    ...
    try {
      const { ceremonyId, options } = await postJson(
...
export function PasskeySignInButton() {
  ...
  const signIn = async () => {
    try {
      ...
      const session = await postJson("/passkeys/authenticate/verify", {
        ceremonyId,
        response: assertion,
      });
