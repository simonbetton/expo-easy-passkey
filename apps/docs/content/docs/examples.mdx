...
The example app calls `apps/example-backend` for WebAuthn options and verification. The committed signing and relying-party values target `expo-easy-passkey.vercel.app`. Replace them before testing native passkey ceremonies against a domain you control:
