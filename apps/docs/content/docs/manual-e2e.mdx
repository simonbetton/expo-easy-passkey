...
- A physical iOS or Android device with screen lock and passkeys enabled. For local Android emulator work before device E2E, sign in with a Google account and set a screen lock so Credential Manager can create and assert passkeys.
